...
# ---------------------------------------------------------------------------
# Same-email, different-role behavior
# ---------------------------------------------------------------------------


async def test_same_email_allows_patient_and_doctor_accounts(client: AsyncClient) -> None:
	email = f"shared_{uuid.uuid4().hex[:8]}@clinsights.dev"
	password = "Password123!"
	...
	doctor_payload = _doctor_signup_payload(email=email)
	with patch("app.api.v1.endpoints.doctor_auth.send_otp_email_task.delay") as mock_delay:
		response = await client.post(f"{API}/signup", json=doctor_payload)
	assert response.status_code == 201, response.text
	assert mock_delay.called
	...
	async with AsyncSessionLocal() as db:
		...
		assert roles == {UserRole.PATIENT, UserRole.DOCTOR}
		for u in users:
			await db.delete(u)
		await db.commit()


async def test_duplicate_doctor_same_email_rejected(client: AsyncClient) -> None:
	email = f"dup_doctor_{uuid.uuid4().hex[:8]}@clinsights.dev"
	await _create_doctor_user(email=email, is_email_verified=True)
