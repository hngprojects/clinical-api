...
_LOGO_PATH = Path(__file__).parent.parent.parent / "ClinsightLogo.svg"


def _load_logo(height_mm: float = 10):
...
def generate_pdf(
	...
	# Resolve report title: manual case title → OCR extracted title → generic fallback
	report_title = case.title
	if not report_title and isinstance(lab_result.extracted_values, dict):
		report_title = lab_result.extracted_values.get("title") or ""
	report_title = escape(report_title.strip()) if report_title and report_title.strip() else "Laboratory Report"

	logo = _load_logo(height_mm=10)
	if logo is not None:
		header_table = Table(
			[[logo, Paragraph("Clinsight", styles["title"])]],
		...
		story.append(Paragraph("Clinsight", styles["title"]))
