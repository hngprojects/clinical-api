...
class BaseMailTransport(ABC):
    @abstractmethod
    async def send(self, subject: str, html: str, to: str, from_email: str | None = None) -> Any:  # pragma: no cover - thin wrapper
...
def _build_smtp_transport() -> SMTPTransport | None:
    ...
    try:
        ...
        port_int = 587
    return SMTPTransport(host=host, port=port_int, username=username, password=password, use_tls=use_tls, from_email=from_email or "", from_name=from_name)
